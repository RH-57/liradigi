...
<head>
  ...
  <title>{{$post->title}} - LIRADIGI.</title>
  ...
  <meta name="twitter:title" content="{{ $post->meta_title }} - Liradigi">
...
<body class="overflow-x-hidden">
  ...
  <header class="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/70 backdrop-blur-md">
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
      <a href="{{ route('home') }}" class="flex items-center gap-1">
        <img src="{{ asset('assets/web/img/logo.png') }}" alt="Logo" class="w-10 h-10 object-contain">
  ...
  <!-- POSTS GRID -->
    <section class="py-20 bg-gradient-to-b from-white to-slate-50 dark:from-slate-950 dark:to-slate-900">
        <div class="max-w-5xl mx-auto px-6">
            {{-- Judul --}}
            <h1 class="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white leading-tight mb-6">
            {{ $post->title }}
            </h1>

            {{-- Info post --}}
            <div class="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mb-10">
            <span><i class="fa-regular fa-calendar mr-2"></i>{{ $post->created_at->format('d M Y') }}</span>
            <span><i class="fa-solid fa-user mr-2"></i>{{ $post->author->name ?? 'Admin' }}</span>
            @if($post->category)
                <span class="px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-300 font-semibold text-xs">
                {{ $post->category->name }}
                </span>
            @endif
            </div>
            ...
            {{-- Gambar utama --}}
            @php
            $img = $post->featured_image ?? $post->cover ?? $post->image ?? null;
            $imgSrc = $img ? asset('storage/' . $img) : asset('assets/web/img/default-cover.jpg');
            @endphp
            <img src="{{ $imgSrc }}" alt="{{ $post->title }}" class="w-full rounded-xl shadow-lg mb-12">

            {{-- Konten artikel --}}
            <div class="prose dark:prose-invert max-w-none leading-relaxed text-lg md:text-xl
            ...
            {{-- Navigasi Next & Prev --}}
            <div class="flex justify-between mt-16 pt-6 border-t border-slate-300 dark:border-slate-700 text-sm">
            @if($prevPost)
                <a href="{{ route('web.posts.show', $prevPost->slug) }}" class="text-emerald-600 hover:underline">&larr; {{ $prevPost->title }}</a>
            @else <span></span> @endif

            @if($nextPost)
                <a href="{{ route('web.posts.show', $nextPost->slug) }}" class="text-emerald-600 hover:underline">{{ $nextPost->title }} &rarr;</a>
            @endif
            </div>
        </div>
    </section>
  ...
  <section id="contact" class="relative py-24 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-950 overflow-hidden">
    <div class="max-w-6xl mx-auto px-6 relative z-10">
      <div class="text-center mb-12">
        <h2 class="text-4xl font-extrabold text-slate-900 dark:text-white">Get in <span class="bg-gradient-to-r from-green-600 via-emerald-500 to-teal-400 bg-clip-text text-transparent">Touch</span></h2>
        <p class="mt-4 text-lg text-slate-600 dark:text-slate-300">Hubungi kami untuk konsultasi gratis atau diskusi project Anda.</p>
      </div>

      <div class="grid lg:grid-cols-2 gap-12 items-center">
        <div class="space-y-3">
          <h3 class="text-2xl font-bold text-slate-900 dark:text-white">Kontak Kami</h3>
          <p class="text-slate-600 dark:text-slate-300">Kami siap membantu Anda dengan solusi digital terbaik. Isi form di samping atau hubungi kami langsung:</p>
          ...
          <form action="{{ route('message.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="email" name="email" class="mt-2 w-full px-4 py-3 rounded-lg bg-white/70 dark:bg-slate-800/70 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
              </div>
            </div>
            <div>
            ...
            <button type="submit" class="w-full px-6 py-3 rounded-lg bg-gradient-to-r from-green-600 via-emerald-500 to-teal-400 text-white font-semibold shadow-md hover:shadow-lg hover:scale-105 transform transition">Kirim Pesan</button>
          </form>
        </div>
      </div>
    </div>
  ...
  <footer class="relative pt-16 pb-10 overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
      ...
      <div class="mt-12 pt-6 border-t border-slate-700 text-center text-sm text-slate-600">© 2025 <strong>Lintas Arah Digital</strong> All rights reserved.</div>
    </div>
  </footer>

  <!-- Floating WhatsApp Button -->
  <a href="https://wa.me/{{ $contacts->phone ?? '' }}?text=Halo%20saya%20tertarik%20dengan%20layanan%20Anda" target="_blank" class="group fixed bottom-6 right-1 z-50 w-16 h-16 rounded-full bg-gradient-to-tr from-green-600 via-emerald-500 to-teal-400 flex items-center justify-center text-white text-3xl shadow-lg hover:scale-110 hover:shadow-2xl transition-all duration-300">
